        ...
        public static string LocationXML()
        {
           ...
           .Where(x => x.Category.CategoryType == CategoryType.Model))  // 1.1.1
            ...
            .Where(x => x.Category.CategoryType == CategoryType.Model))
            {
                ...
                {
                    if (e.Category.Id.GetValue().Equals((int)BuiltInCategory.OST_Columns) || e.Category.Id.GetValue().Equals((int)BuiltInCategory.OST_StructuralColumns))
                    {
                    ...
                    {
                        if (!e.Category.Id.GetValue().Equals((int)BuiltInCategory.OST_PipeFitting) ||
                            !e.Category.Id.GetValue().Equals((int)BuiltInCategory.OST_DuctFitting) ||
                            !e.Category.Id.GetValue().Equals((int)BuiltInCategory.OST_ConduitFitting) ||
                            !e.Category.Id.GetValue().Equals((int)BuiltInCategory.OST_CableTrayFitting))  // 1.1.1
                        {
...
#pragma warning restore CS0219 // The variable 'regSystem' is assigned but its value is never used
                    ...
                    if (multipoint != "<None>" && multipoint != "" && multipoint != null)
                    {
                        ...
                        try
                        {
                            ...
                            if (internalElement is Floor)
                            {
                                ...
                                var levelId = element.InternalElement.Parameters.Cast<Parameter>().First(x => x.Id.GetValue().Equals((int)BuiltInParameter.LEVEL_PARAM)).AsElementId();  // 1.1.0
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (element.InternalElement.Location is LocationPoint) // Vertical column
                                {
                                    ...
                                    ElementId baseId = element.InternalElement.Parameters.Cast<Parameter>().First(x => x.Id.GetValue().Equals((int)BuiltInParameter.FAMILY_BASE_LEVEL_PARAM)).AsElementId();

                                    Level bl = (Level)doc.GetElement(baseId);

                                    double bh = bl.Elevation;

                                    ElementId topId = element.InternalElement.Parameters.Cast<Parameter>().First(x => x.Id.GetValue().Equals((int)BuiltInParameter.FAMILY_TOP_LEVEL_PARAM)).AsElementId();
                                    ...
                                    element.InternalElement.Parameters.Cast<Parameter>()
                                        .First(x => x.Id.GetValue().Equals((int)BuiltInParameter.FAMILY_BASE_LEVEL_OFFSET_PARAM)).Set(Utils.MToFeet(elevation) - bh);

                                    element.InternalElement.Parameters.Cast<Parameter>()
                                        .First(x => x.Id.GetValue().Equals((int)BuiltInParameter.FAMILY_TOP_LEVEL_OFFSET_PARAM)).Set(Utils.MToFeet(endElevation) - th);
                                } // Vertical columns
                                else
                                {
        ...
        public static void UpdateMEPCurve(Element element, Autodesk.Revit.DB.Curve newCurve)
        {
            ...
            foreach (Connector connector in cm.Connectors)
            {
                ...
                {
                    if (eCon.Owner.Id.GetValue() != p.Id.GetValue())
                    {
                        ...
                        if (eCon.Owner is FamilyInstance)
                        {
                            ...
                            {
                                if (qCon.Owner.Id.GetValue() != eCon.Owner.Id.GetValue() &&
                                   qCon.Owner.Id.GetValue() != p.Id.GetValue())
                                {
        ...
        public static object[][] ReadFamilyInstancesPointBased(Document doc, CivilDocument civilDocument, int categoryId)
        {
            ...
            var instances = new FilteredElementCollector(doc)
                ...
                .Where(x => x.ViewSpecific == false &&
                    ...
                    x.Category.Id.GetValue().Equals(categoryId));
            ...
            foreach (FamilyInstance fi in instances)
            {
                ...
                Parameter pMark = fi.Parameters.Cast<Parameter>().First(g => g.Id.GetValue().Equals(Convert.ToInt32(BuiltInParameter.ALL_MODEL_MARK)));
        ...
        public static object[] LinearObjectLocationParameters(object linearMEPCurve)
        {
            ...
            var typeName = Revit.Elements.ElementSelector.ByElementId((int)typeId.GetValue()).Name;
        ...
        /// <returns></returns>
        public static ExternalDefinition RawCreateProjectParameter(Document doc, ADSK_Parameter parameter, ForgeTypeId type, bool visible, bool userModifiable, CategorySet cats, ForgeTypeId group, bool inst) // updated to Revit 2025
        {
            ...
            foreach (Category cat in cats)
            {
                ...
                while (iter.MoveNext())
                {
                    if (iter.Key.Name == name)
                    {
                        ...
                        {
                            if (catEB.Id.GetValue().Equals(cat.Id.GetValue()))
                            {
                                if (type == iter.Key.GetDataType())
                                {
                                    if (group == iter.Key.GetGroupTypeId())
                                    {
                                        found = false;
                                        break;
                                    }
        ...
        public static void CheckParameters(Document doc)
        {
            ...
            if (!SessionVariables.ParametersCreated)
            {
                ...
                RawCreateProjectParameter(doc, par.Corridor, SpecTypeId.String.Text, true, true, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.BaselineIndex, SpecTypeId.Int.Integer, true, true, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.RegionIndex, SpecTypeId.Int.Integer, true, false, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.RegionRelative, SpecTypeId.Length, true, false, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.RegionNormalized, SpecTypeId.Number, true, false, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.Code, SpecTypeId.String.Text, true, true, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.Side, SpecTypeId.String.Text, true, true, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.X, SpecTypeId.Length, true, false, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.Y, SpecTypeId.Length, true, false, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.Z, SpecTypeId.Length, true, false, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.Station, SpecTypeId.Length, true, true, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.Offset, SpecTypeId.Length, true, true, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.Elevation, SpecTypeId.Length, true, true, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.AngleZ, SpecTypeId.Angle, true, true, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.Update, SpecTypeId.Boolean.YesNo, true, true, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.Delete, SpecTypeId.Boolean.YesNo, true, true, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.MultiPoint, SpecTypeId.String.Text, true, true, cs, GroupTypeId.Data, true);
                ...
                foreach (Category cat in doc.Settings.Categories.Cast<Category>().Where(c => c.AllowsBoundParameters && c.CategoryType == CategoryType.Model))
                {
                    ...
                    if (//cat.Id.IntegerValue.Equals((int)BuiltInCategory.OST_PipeSegments) ||
                        cat.Id.GetValue().Equals((int)BuiltInCategory.OST_PipeCurves) ||
                        cat.Id.GetValue().Equals((int)BuiltInCategory.OST_FlexPipeCurves) ||
                        cat.Id.GetValue().Equals((int)BuiltInCategory.OST_DuctCurves) ||
                        cat.Id.GetValue().Equals((int)BuiltInCategory.OST_FlexDuctCurves) ||
                        cat.Id.GetValue().Equals((int)BuiltInCategory.OST_PlaceHolderDucts) ||
                        cat.Id.GetValue().Equals((int)BuiltInCategory.OST_PlaceHolderPipes) ||
                        cat.Id.GetValue().Equals((int)BuiltInCategory.OST_CableTray) ||
                        cat.Id.GetValue().Equals((int)BuiltInCategory.OST_Conduit) ||
                        cat.Id.GetValue().Equals((int)BuiltInCategory.OST_StructuralFraming) ||
                        cat.Id.GetValue().Equals((int)BuiltInCategory.OST_StructuralColumns) ||
                        cat.Id.GetValue().Equals((int)BuiltInCategory.OST_Columns) ||
                        cat.Id.GetValue().Equals((int)BuiltInCategory.OST_Walls))
                    {
                        cs.Insert(cat);
                    }
                }

                RawCreateProjectParameter(doc, par.EndStation, SpecTypeId.Length, true, true, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.EndOffset, SpecTypeId.Length, true, true, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.EndElevation, SpecTypeId.Length, true, true, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.EndRegionRelative, SpecTypeId.Length, true, false, cs, GroupTypeId.Data, true);
                RawCreateProjectParameter(doc, par.EndRegionNormalized, SpecTypeId.Number, true, false, cs, GroupTypeId.Data, true);
                ...
                foreach (int i in updateIds)
                {
                    if (null != data[i][0])
                    {
                        ...
                        if (location is LocationPoint)
                        {
                            ...
                            Parameter pMark = fi.Parameters.Cast<Parameter>().First(p => p.Id.GetValue().Equals(Convert.ToInt32(BuiltInParameter.ALL_MODEL_MARK)));
...
#pragma warning restore CS0219 // The variable 'useFeatureLine' is assigned but its value is never used
        ...
        public static string RevitLinkByStationOffsetElevation(Revit.Elements.Element revitLinkType, Featureline featureline, double station, double offset = 0, double elevation = 0, bool rotate = true, double rotation = 0)
        {
            ...
            foreach (RevitLinkInstance i in new FilteredElementCollector(doc)
                 ...
                 .Where(x => x.GetTypeId().GetValue().Equals(revitLinkType.InternalElement.Id.GetValue())))
            {
                if (i.Parameters.Cast<Parameter>().First(x => x.Id.GetValue().Equals((int)BuiltInParameter.RVT_LINK_INSTANCE_NAME)).AsString() == name)
                {
                    found = true;
                    rli = i;
                    break;
            ...
            rli.Parameters.Cast<Parameter>().First(x => x.Id.GetValue().Equals((int)BuiltInParameter.RVT_LINK_INSTANCE_NAME)).Set(name);
        ...
        public static Revit.Elements.Wall WallBySurface(Autodesk.DesignScript.Geometry.Surface surface, Revit.Elements.WallType wallType, bool structural = false)
        {
            ...
            if (!PolyCurve.ByJoinedCurves(surface.PerimeterCurves(),0.001, false, 0).IsPlanar)
            {
            ...
            Utils.Log(string.Format("New wall created {0}...", newWall.Id.GetValue()));
            ...
            try
            {
                ...
                newWall.Parameters.Cast<Parameter>().First(x => x.Id.GetValue() == (int)BuiltInParameter.WALL_BASE_OFFSET).Set(min - level.Elevation);
            ...
            try
            {
                newWall.Parameters.Cast<Parameter>().First(x => x.Id.GetValue() == (int)BuiltInParameter.WALL_HEIGHT_TYPE).Set(ElementId.InvalidElementId);
            ...
            try
            {
                newWall.Parameters.Cast<Parameter>().First(x => x.Id.GetValue() == (int)BuiltInParameter.WALL_USER_HEIGHT_PARAM).Set(max - min);
            ...
            try
            {
                newWall.Parameters.Cast<Parameter>().First(x => x.Id.GetValue() == (int)BuiltInParameter.WALL_KEY_REF_PARAM).Set(0);
